...
{
	public class LeafNameOnlyAncestorReport : IBaseReport
	{
		...
		public LeafNameOnlyAncestorReport()
		{
		...
		private void OutputReport(string rootIndividialId, string outputPath)
		{
			using (StreamWriter writer = new StreamWriter(outputPath))
			{
				...
				writer.WriteLine(string.Format("Abrdiged Leaf Ancestor Report for {0}", ancestors[rootIndividialId].SummaryName));
				writer.WriteLine(string.Format("Generated on {0}", DateTime.Now.ToShortDateString()));
				writer.WriteLine(string.Format("Total ancestors in report {0}", ancestors.Where(x => string.IsNullOrEmpty(x.Value.MotherId) || string.IsNullOrEmpty(x.Value.MotherId)).Count()));
				writer.WriteLine();
		...
		private void CalculateAncestorByGenerationDictionary()
		{
			...
			{
				optimizedAncestors.Add(i, ancestors.Values.Where(x => x.HighestGeneration == i && (String.IsNullOrEmpty(x.FatherId) || String.IsNullOrEmpty(x.MotherId))).ToList());
			}
		}
